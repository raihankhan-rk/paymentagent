...
async function saveWalletData(userId: string, walletData: string) {
  const localFilePath = `${STORAGE_DIR}/wallets/${userId}.json`;
  try {
 ...
 */
async function searchMemories(userId: string, query: string): Promise<SearchResult[]> {
...
async function handleMessage(message: DecodedMessage, client: Client) {
  try {
    ...
    const searchResults = await memoryClient.search(message.content, { user_id: senderAddress });
    ...
    let messageWithContext = message.content;
    if (searchResults && searchResults.length > 0) {
      const relevantMemories = searchResults
        .filter(result => result.score > 0.3) // Only use memories with good relevance
        .map(result => result.memory)
        .join('\n');
      messageWithContext = `Context from previous conversations:\n${relevantMemories}\n\nCurrent message: ${message.content}`;
    }
    console.log(messageWithContext);

    const response = await processMessage(agent, config, messageWithContext);
